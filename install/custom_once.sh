...
if [ ! -f ${CONFIG_FILE} ]; then
    # add configs
    yes | cp -rf "${INSTALL_DIR}/config.php" ${CONFIG_FILE}

    # FIX Moodle sitename for CLI scripts
    sed -i "s/MOODLE_HTTP_HOST/${VIRTUAL_HOST}/" ${CONFIG_FILE}

    # config db
    if [ ! -z "${DB_HOST}" ]
    then
        # db host
        sed -i "s/MOODLE_DB_HOST/${DB_HOST}/" ${CONFIG_FILE}
        # db name
        if [ -z "${DB_NAME}" ]
        then
            DB_NAME=${PROJECT}
        fi
        sed -i "s/MOODLE_DB_NAME/${DB_NAME}/" ${CONFIG_FILE}
    fi
fi
